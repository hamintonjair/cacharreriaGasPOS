...
toast.success = (message, duration = 3000) => toast(message, 'success', duration)
...
export default function Users() {
  ...
  const validateForm = () => {
    ...
    if ((!formData.password && modal.mode === 'create') || (formData.password && formData.password.length < 6)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || `Error al ${modal.mode === 'edit' ? 'actualizar' : 'crear'} el usuario`)
      }
      
      toast.success(`Usuario ${modal.mode === 'edit' ? 'actualizado' : 'creado'} correctamente`)
      loadUsers()
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm(`¿Estás seguro de eliminar al usuario "${modal.record.nombre}"? Esta acción no se puede deshacer.`)) {
      return
    }
    
    try {
      const userId = String(modal.record.id)
      ...
      toast.error(error.message || 'Error al eliminar el usuario. ' + 
        (error.message.includes('asociados') ? error.message : 'Por favor, inténtalo de nuevo.'))
    }
  }
  ...
  return (
    <section className="relative p-4 border rounded-xl bg-white">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        ...
        <p className="text-gray-600 mt-2">Administra el registro de usuarios del sistema</p>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        <div className="text-sm text-gray-500">Página {currentPage} de {totalPages} — {filteredAndSortedUsers.length} elementos</div>
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <form onSubmit={handleSubmit}>
                      ...
                      <div className="mt-4 space-y-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`mt-1 block w-full border ${formErrors.nombre ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm`}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleInputChange}
                            className={`mt-1 block w-full border ${formErrors.username ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm`}
                        ...
                        <div>
                          <label htmlFor="password" className="block text-sm font-medium text-gray-700">
                            {modal.mode === 'create' ? 'Contraseña' : 'Nueva Contraseña (dejar en blanco para no cambiar)'}
                          </label>
                          <input
                            type="password"
                            name="password"
                            ...
                            onChange={handleInputChange}
                            className={`mt-1 block w-full border ${formErrors.password ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm`}
                            placeholder={modal.mode === 'create' ? 'Mínimo 6 caracteres' : 'Dejar en blanco para no cambiar'}
                          />
                        ...
                        <div>
                          ...
                          <select
                            ...
                            value={formData.role}
                            onChange={handleInputChange}
                            className={`mt-1 block w-full border ${formErrors.role ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm`}
                          >
                            {ROLES.map(role => (
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  ...
                  <button
                    type="button"
                    onClick={() => setModal({ open: false, mode: null, record: null })}
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        ¿Estás seguro de que deseas eliminar al usuario <span className="font-medium">{modal.record?.nombre || modal.record?.username}</span>? Esta acción no se puede deshacer.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setModal({ open: false, mode: null, record: null })}
                  className="h-10 px-4 rounded bg-gray-100 hover:bg-gray-200 transition-colors duration-200"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="h-10 px-4 rounded bg-gray-900 text-white hover:bg-gray-800 transition-colors duration-200"
                >
                  Eliminar
                </button>
              </div>
